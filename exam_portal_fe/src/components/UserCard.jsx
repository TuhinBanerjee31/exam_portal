...
const UserCard = (props) => {
  ...
  const toggleResult = async () => {
    console.log(props.data.showResult)
    try {
      ...
      const headers = {
        ...
        body: JSON.stringify({
          _id: props.data._id,
          showResult: !props.data.showResult,
        }),
      };
      const response = await fetch(url, headers);
      const result = await response.json();
  ...
  return (
    <div className="w-96 p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
      ...
      <div className="flex font-medium items-center gap-4 justify-around pt-2 text-blue-600 hover:underline">
        <Link to={"/admin/view-user"} state={props.data}>
        ...
        <span className="cursor-pointer" onClick={toggleResult}>
          {props.data.showResult ? <LockKeyholeOpen /> : <LockKeyhole />}
        </span>
      </div>
